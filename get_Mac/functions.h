...
int print_beacon(beaconinfo beacon)
{
  if (beacon.err != 0) {
...
char* print_client(clientinfo ci)
{
  int u = 0;
  int known = 0;   // Clear known flag
  char clientMac[60] = "ffffffffffff";
  ...
  if (ci.err != 0) {
    ...
    for (int i = 0; i < 6; i++) sprintf(clientMac+2*i,"%02x", ci.station[i]);
    Serial.printf(" ==> ");
